...
| 분석 | GA4 (gtag.js) | 2026-07-12 사용자 결정. 표준 분석 도구, 배너 없이 로드 (아래 분석/컴플라이언스 참조) |
...
| 2026-07-12 | Google Analytics 4(gtag.js) 도입 | Measurement ID `G-MW73GQ2ZRX` 를 `app/[locale]/layout.tsx` `<body>` 최상단에 `next/script`(afterInteractive) 2개(gtag 로더 + init)로 로드 — raw `<head>` 붙여넣기 대신 App Router 표준 방식(하이드레이션 안전, 3 locale 전 페이지 자동 적용). 공개 토큰이라 하드코딩(naver-site-verification 패턴). **사용자 결정으로 기존 "쿠키 분석 금지(GA4 비권장)" 가드레일 완화** — "GA4 는 표준이니 일반적 방향, 충돌 시 가드레일 완화" 명시. 배너 없이 즉시 로드(1차 타겟 한·일 + 마케팅 랜딩, 개인식별 데이터 미수집). 검증: web typecheck PASS. **후속(P1)**: `content/legal/privacy.{ko,ja,en}.md` 에 GA 쿠키(`_ga`) 조항 추가 / EU 트래픽 유의미해지면 Consent Mode v2 + 동의 배너로 승격(safety 사인오프) |
