...
// profile(기본정보)로 진행하며, 동의 기록은 프로필 최초 생성(photos upsert) 시 BE 가
...
export default function SetupConsent() {
  ...
  // Wizard entry: swipe-back / hardware-back = logout. (profile 의 진입 가드를 첫
  // 단계인 본 화면으로 이관 — profile 은 그대로 두어 그 이후 단계 동작 무변경.)
  useFocusEffect(
    useCallback(() => {
  ...
  const handleSubmit = () => {
    setConsentAccepted();
    router.push('/(main)/setup/profile');
  };

  // 진입 첫 단계라 헤더 back 화살표는 두지 않는다(profile 의 WizardHeader 와 동일 —
